  ...
  <div class="app-container">
    ...
    <el-card class="box-card garmin-form">
      <el-form :inline="true" :model="searchField">
        ...
        <el-form-item>
          <el-input v-model="searchField.name" placeholder="搜索商品名称"></el-input>
        </el-form-item>
    ...
    >
      ...
      <el-table-column label="商品图片" width="110" align="center">
        <template slot-scope="scope">
          ...
          <el-avatar shape="square" :size="100" fit="fill"  :src="scope.row.thumb" />
  ...
  export default {
    ...
    methods: {
      ...
      fetchData() {
        ...
        self.$request({url:'/goods/goods', method:'GET', params}).then(response=>{
          if (response.data.list){
            self.list =  response.data.list.forEach(item=>{
              item.thumb = self.img_domain + item.thumb;
              return item;
            })
          }
      ...
      editOrCreate(id=0){
        console.log(id)
        this.$router.push({path:'/goods/goods/add_edit', query: {id:id}})
        // self.currentId = id;
